...
async fn handle_responses_for_app(
    ...
    let codex_continue_native_only = matches!(app_type, AppType::Codex)
        && !providers.iter().any(|provider| {
            super::providers::should_convert_codex_responses_to_chat(provider, &endpoint)
        });
    let codex_continue_config = if codex_continue_native_only {
        Some(
            state
                .db
    ...
    let codex_continue_enabled = codex_continue_config
        .as_ref()
        .is_some_and(|config| super::codex_continue::should_enable_for_request(&body, config));
    ...
    if let Some(codex_continue_config) = codex_continue_config {

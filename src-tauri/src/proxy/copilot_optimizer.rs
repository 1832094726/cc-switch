...
pub fn strip_thinking_blocks(mut body: Value) -> Value {
    strip_cc_switch_thinking_tags(&mut body);
...
/// Strip legacy `<cc-switch:thinking>...</cc-switch:thinking>` text tags from all
/// message content. These tags were generated by older builds and may persist in
/// cached conversation history that the client replays as plain text.
fn strip_cc_switch_thinking_tags(body: &mut Value) {
    const TAG_OPEN: &str = "<cc-switch:thinking>";
    const TAG_CLOSE: &str = "</cc-switch:thinking>";

...
impl RequestForwarder {
    ...
    > {
        ...
        let codex_model_catalog_to_chat = model_catalog_route.as_ref().is_some_and(|route| {
            route
                .endpoint
                .as_deref()
        ...
        let codex_anthropic_upstream = is_joycode_anthropic_route || codex_responses_to_anthropic;
        // Codex -> Anthropic emulation is opt-in so generic gateways do not
        // receive Claude Code's fingerprint and identity prompt unexpectedly.
        ...
        if status.is_success() {
            let mut response = self
                ...
                .await?;
            // Validate semantic failures before exposing a successful response.
...
fn should_use_direct_http_client_for_upstream(
    ...
    .unwrap_or(false)
        ...
        || {
            ...
            let config_lower = provider.settings_config.to_string().to_ascii_lowercase();
...
fn elided_digest_regex() -> &'static Regex {
    static REGEX: OnceLock<Regex> = OnceLock::new();
    REGEX.get_or_init(|| Regex::new(r"(?i)elided:\s*chars=\d+,\s*sha256=[0-9a-f]+").unwrap())
}
...
wire_api = "responses""#,
    ...
    fn codex_anthropic_full_endpoint_guard_avoids_double_messages() {
        ...
        let unguarded = super::super::providers::CodexAdapter::new()
            .build_url("https://host.example/v1/messages", "/v1/messages");
        assert_ne!(unguarded, "https://host.example/v1/messages");
